...
class DepthEstimator:
    ...
    def _estimate(self, img_numpy):
        ...
        return F.interpolate(prediction.unsqueeze(1), size=img_numpy.shape[:2], mode="bicubic", align_corners=False).squeeze()
...
class RefinedGaussianModel(GaussianModel):
    ...
    def reset_iter_counters(self):
        for key in self.event_counters:
            if key.endswith("_Iter"): self.event_counters[key] = 0
    ...
    def training_setup(self, training_args):
        ...
        for group in self.optimizer.param_groups:
            if group["name"] == "importance":
                ...
                found = True
        if not found:
            self.optimizer.add_param_group({'params': [self._importance], 'lr': training_args.feature_lr * 0.1, "name": "importance"})
    ...
    def floater_cleanup(self, visibility_threshold, grad_threshold, opacity_threshold,
                        importance_threshold, max_fraction, distance_factor=1.5):
...
def validation_loop(iteration, scene, gaussians, pipe, background, depth_estimator):
    ...
    for viewpoint_cam in tqdm(scene.getTestCameras(), desc="Validation"):
        ...
        with torch.no_grad():
            render_pkg = render(viewpoint_cam, gaussians, pipe, background, importance_scores=torch.sigmoid(gaussians.get_importance))
        ...
        if gt_depth is not None:
            ...
            if gt_depth.shape != rendered_depth.shape:
                 gt_depth = F.interpolate(gt_depth.unsqueeze(0).unsqueeze(0), size=rendered_depth.shape, mode='bilinear', align_corners=False).squeeze()
    ...
    metrics = {
        'Val/PSNR': np.mean(psnr_list), 'Val/SSIM': np.mean(ssim_list),
        'Val/LPIPS': np.mean(lpips_list), 'Val/L1': np.mean(l1_list),
    ...
    print(f"[ITER {iteration}] Validation: PSNR {metrics['Val/PSNR']:.2f}, LPIPS {metrics['Val/LPIPS']:.4f}, Depth Abs-Rel {metrics['Val/DepthAbsRel']:.4f}")
    return metrics
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter + 1, opt.iterations + 1):        
        ...
        render_pkg = render(viewpoint_cam, gaussians, pipe, background, importance_scores=torch.sigmoid(gaussians.get_importance))
        image, rendered_depth, visibility_filter = render_pkg["render"], render_pkg["depth"], render_pkg["visibility_filter"]
        ...
        if gt_depth is not None:
            ...
            if gt_depth.shape != rendered_depth_no_channel.shape:
                 gt_depth = F.interpolate(gt_depth.unsqueeze(0).unsqueeze(0), size=rendered_depth_no_channel.shape, mode='bilinear', align_corners=False).squeeze()
            ...
            if valid_depth_mask.sum() > 0:
                depth_loss = (weights[valid_depth_mask] * F.huber_loss(rendered_depth_aligned[valid_depth_mask], gt_depth[valid_depth_mask], reduction='none')).mean()
        ...
        with torch.no_grad():
            if iteration < opt.refinement_start_iter:
                if iteration < opt.densify_until_iter:
                    ...
                    if iteration % opt.opacity_reset_interval == 0: gaussians.reset_opacity()
            else:
        ...
        if eval_due: # Time to run validation/logging
            ...
            if iteration in saving_iterations:
                ...
                if os.path.exists(ply_path): model_size_mb = os.path.getsize(ply_path) / (1024 * 1024)

            total_pruned_iter = (
                gaussians.event_counters.get('Pruned(Densify)_Iter', 0) +
                gaussians.event_counters.get('Pruned(Final)_Iter', 0)
            )
            total_pruned_cum = (
                gaussians.event_counters.get('Pruned(Densify)_Cum', 0) +
                gaussians.event_counters.get('Pruned(Final)_Cum', 0)
            )
        ...
        if iteration in checkpoint_iterations: torch.save((gaussians.capture(), iteration), scene.model_path + f"/chkpnt{iteration}.pth")
...
def prepare_output_and_logger(args):    
    ...
    header = [
        ...
        'Train/L1', 'Train/TotalLoss', 'Train/SSIM', 'RenderFPS', 'IterTime(ms)', 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lambda_importance", type=float, default=0.01, help="Weight for the differentiable pruning regularization.")
    parser.add_argument("--uncertainty_tau", type=float, default=0.1, help="Temperature for uncertainty weighting.")
    parser.add_argument("--cleanup_interval", type=int, default=100, help="Interval for cleanup/regularization in Stage 2.")
    parser.add_argument("--floater_cleanup_start", type=int, default=2000, help="Iteration to start floater cleanup.")
    parser.add_argument("--floater_cleanup_interval", type=int, default=400, help="Interval between floater cleanup passes.")
    parser.add_argument("--floater_visibility_threshold", type=int, default=2, help="Minimum visibility count before a Gaussian is considered a floater.")
    parser.add_argument("--floater_grad_threshold", type=float, default=5e-4, help="Maximum gradient EMA for floater candidates.")
    parser.add_argument("--floater_opacity_threshold", type=float, default=0.04, help="Maximum opacity for floater candidates.")
    parser.add_argument("--floater_importance_threshold", type=float, default=0.35, help="Maximum learned importance score for floater candidates (after sigmoid).")
    parser.add_argument("--floater_cleanup_max_ratio", type=float, default=0.05, help="Maximum fraction of Gaussians that can be removed in a single floater cleanup.")
    parser.add_argument("--floater_distance_factor", type=float, default=1.4, help="Distance factor relative to scene extent for floater detection.")
    parser.add_argument("--max_gaussians", type=int, default=180000, help="Upper bound on active Gaussian count (0 to disable).")
    parser.add_argument("--eval_interval", type=int, default=200, help="Run validation and log metrics every N iterations (0 to disable periodic eval).")
    ...
    training(dataset_params, opt_params, pp.extract(args),

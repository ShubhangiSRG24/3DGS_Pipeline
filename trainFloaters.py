...
class RefinedGaussianModel(GaussianModel):
    ...
    def _ensure_aux_lengths(self):
        ...
        self.xyz_gradient_accum = _match(
            ...
            (self.xyz_gradient_accum.shape[1],) if self.xyz_gradient_accum is not None else (1,),
        )
        self.denom = _match(
            self.denom,
    ...
    def set_dynamic_thresholds(self):
        ...
        counts = torch.zeros(
            G * G * G, device=self.get_xyz.device, dtype=torch.float32
        )
        w = (
            self.merge_weights
    ...
    def _fast_merge_and_apply(
        ...
        idx_within = torch.arange(N, device=device) - torch.repeat_interleave(
            torch.cumsum(
                torch.cat(
                    [torch.tensor([0], device=device), counts_per_cell[:-1]]
                ),
                dim=0,
            ),
            counts_per_cell,
        ...
        posA_sorted = torch.nonzero(
            (idx_within % 2 == 0)
            & (idx_within + 1 < counts_per_cell[seg_ids]),
            as_tuple=False,
        ).squeeze(-1)
    ...
    def refine_and_prune(
        ...
        if global_prune_mask.any():
            ...
            prune_candidates = torch.nonzero(
                global_prune_mask, as_tuple=False
            ).squeeze(-1)
    ...
    def floater_cleanup(
        ...
        with torch.no_grad():
            ...
            if idx.numel() > prune_cap:
                _, order = torch.topk(
                    importance[idx], k=prune_cap, largest=False
                )
                idx = idx[order]
...
def color_moment_loss(pred, tgt, eps=1e-6):
    ...
    ps, qs = torch.sqrt(pf.var(-1, unbiased=False) + eps), torch.sqrt(
        qf.var(-1, unbiased=False) + eps
    )
...
def validation_loop(iteration, scene, gauss, pipe, bg, depth_est):
    ...
    for vcam in tqdm(cams, desc="[Val]"):
        ...
        start = time.time()
        with torch.no_grad(), suppress_debug_lines(
            ("DEBUG:", "EARLY DEBUG:", "RASTERIZER DEBUG:")
        ):
            pkg = render(
                vcam,
                gauss,
...
def training(dataset, opt, pipe, test_iters, save_iters, ckpt_iters, checkpoint, debug_from):
    ...
    depth_est = DepthEstimator(
        cache_dir=os.path.join(scene.model_path, "depth_cache")
    )
    ...
    for it in range(first_iter + 1, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            if it < opt.densify_until_iter:
                ...
                gauss.max_radii2D[vis] = torch.max(
                    gauss.max_radii2D[vis], radii[vis]
                )
                gauss.add_densification_stats(pkg["viewspace_points"], vis)
                fast_interval = max(
                    1, int(getattr(opt, "densification_interval", 100))
                )
                ...
                if it % 200 == 0:
                    gauss._ensure_point_buffers()
                    radii = align_tensor_to_length(
                        pkg["radii"], gauss.get_xyz.shape[0]
                    )
                ...
                if it % 500 == 0:
                    low_imp = (
                        torch.sigmoid(gauss.get_importance) < 0.05
                    ).squeeze()
                    if low_imp.any():
            ...
            if (
                ...
                gauss.floater_cleanup(
                    ...
                    grad_threshold=getattr(
                        opt, "floater_grad_threshold", 5e-4
                    ),
                    opacity_threshold=getattr(
                        opt, "floater_opacity_threshold", 0.04
                    ),
                    importance_threshold=getattr(
                        opt, "floater_importance_threshold", 0.35
                    ),
                    max_fraction=getattr(
                        opt, "floater_cleanup_max_ratio", 0.05
                    ),
                    distance_factor=getattr(
                        opt, "floater_distance_factor", 1.4
                    ),
                )
                if getattr(opt, "max_gaussians", 0) > 0:
                    gauss.cap_gaussian_count(
                        getattr(opt, "max_gaussians", 0)
                    )
        ...
        base_row = {
            ...
            "Train/Grad": float(grad_loss.item())
            if grad_loss.numel()
            else 0.0,
            ...
            "Train/Depth": float(depth_loss.item())
            if depth_loss.numel()
            else 0.0,
        ...
        if do_eval:
            ...
            pc_dir = os.path.join(
                scene.model_path, "point_cloud", f"iteration_{it}"
            )
            size_mb = 0.0
            if os.path.isdir(pc_dir):
                size_mb = (
                    sum(
                        os.path.getsize(p)
                        for p in glob.glob(os.path.join(pc_dir, "*.ply"))
                    )
                    / (1024 * 1024)
                )
            if size_mb > 0:
        ...
        csv_writer.writerow(
            {k: base_row.get(k, "N/A") for k in csv_writer.fieldnames}
        )
        csv_file.flush()
...
if __name__ == "__main__":
    ...
    safe_add_argument(
        parser, "--floater_cleanup_max_ratio", type=float, default=0.05
    )

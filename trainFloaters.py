# === train_unified.py (20k fixed, multi-res, boosted densify, region-refine, floater cleanup, depth eval, FLAT CSV) ===
...
try:
    ...
    if torch.cuda.is_available(): _lpips = _lpips.cuda()
    _lpips.eval(); _HAS_LPIPS = True
except Exception:
    _lpips = None; _HAS_LPIPS = False
...
def align_tensor_to_length(tensor: torch.Tensor, target_len: int):
    if tensor.shape[0] == target_len: return tensor
    if tensor.shape[0] > target_len:  return tensor[:target_len]
...
def to_chw01(img: torch.Tensor) -> torch.Tensor:
    ...
    if img.ndim == 3:
        if img.shape[-1] in (3,4): return img[..., :3].permute(2,0,1).unsqueeze(0).float().contiguous()
        if img.shape[0] in (3,4):  return img[:3].unsqueeze(0).float().contiguous()
    if img.ndim == 4:
        if img.shape[-1] in (3,4): return img[..., :3].permute(0,3,1,2).float().contiguous()
        if img.shape[1] in (3,4):  return img[:,:3].float().contiguous()
...
def safe_add_argument(parser: ArgumentParser, *flags, **kwargs):
    ...
    if any(f in existing for f in flags): return
    parser.add_argument(*flags, **kwargs)

def _append_if_finite(lst, v):
    if isinstance(v, (float, int)) and math.isfinite(v): lst.append(float(v))
...
class DepthEstimator:
    ...
    def _p(self, image_name, suffix): return os.path.join(self.cache_dir, os.path.basename(image_name)+f"_{suffix}.pt")

    @torch.no_grad()
    def _estimate(self, img_numpy):
        x = self.transform(img_numpy).to(self.device)
        pred = self.model(x)
        return F.interpolate(pred.unsqueeze(1), size=img_numpy.shape[:2], mode="bicubic", align_corners=False).squeeze()
    ...
    def precompute(self, cams):
        ...
        for cam in tqdm(cams, desc="[Depth] Precompute"):
            dp, up = self._p(cam.image_name,"depth"), self._p(cam.image_name,"uncertainty")
            if os.path.exists(dp) and os.path.exists(up): continue
            pil = to_pil_image(cam.original_image.cpu())
            img = np.array(pil); img_f = np.array(hflip(pil))
            d1 = self._estimate(img); d2 = torch.fliplr(self._estimate(img_f))
            ...
            torch.save(mean.cpu(), dp); torch.save(var.cpu(), up)
...
class RefinedGaussianModel(GaussianModel):
    ...
    def training_setup(self, training_args):
        ...
        n = self.get_xyz.shape[0]; dev = self.get_xyz.device
    ...
    def _ensure_point_buffers(self):
        ...
        def _align(t, fill=0.0):
            ...
            if t.shape[0] > n: return t[:n]
            pad = torch.zeros((n - t.shape[0],) + t.shape[1:], device=dev, dtype=t.dtype)
        ...
        if self.tmp_radii is None: self.tmp_radii = torch.zeros(n, device=dev)
        else:
            if self.tmp_radii.device != dev: self.tmp_radii = self.tmp_radii.to(dev)
            if self.tmp_radii.shape[0] != n:
                new_tmp = torch.zeros(n, device=dev); c = min(self.tmp_radii.shape[0], n)
                if c>0: new_tmp[:c] = self.tmp_radii[:c]
    ...
    def _limit_selected_points(self, selected_mask, scores, max_points):
        ...
        if scores is None:
            ...
            _, order = torch.topk(s, k=k, largest=True); chosen = idx[order]
        m = torch.zeros_like(selected_mask); m[chosen] = True
        return m
    ...
    def _ensure_aux_lengths(self):
        ...
        self.xyz_gradient_accum = _match(self.xyz_gradient_accum, (self.xyz_gradient_accum.shape[1],) if self.xyz_gradient_accum is not None else (1,))
        self.denom = _match(self.denom, (self.denom.shape[1],) if self.denom is not None else (1,))
        ...
        if target <= 0:
            ...
            if self._importance is None:
                ...
                if cur.shape[0] > target:
                    ...
                    pad = torch.ones(target - cur.shape[0], 1, device=dev, dtype=cur.dtype) * 2.0
    ...
    def densify_and_clone(self, grads, th, extent):
        ...
        scaling_ok = torch.max(self.get_scaling, dim=1).values <= self.percent_dense * extent
        candidate = torch.logical_and(gnorm >= th, scaling_ok)
        sel = candidate
        cap = int(getattr(self.training_args, "max_clone_points_per_iter", 0) or 0)
        if cap > 0: sel = self._limit_selected_points(candidate, gnorm, cap)
    ...
    def densify_and_split(self, grads, th, extent, N=2):
        ...
        scale_mask = torch.max(self.get_scaling, dim=1).values > self.percent_dense * extent
    ...
    def densification_postfix(self, new_xyz, f_dc, f_rest, opac, scaling, rot, new_tmp_radii):
        ...
        if self.tmp_radii is None: self.tmp_radii = new_tmp_radii
        else:
    ...
    # ------------------------ Region labeling & refine ------------------------
    def set_dynamic_thresholds(self):
        if self.get_xyz.shape[0] == 0: return
        ...
        initial_grid_size = max(4, min(64, int((num_points / max(target_points_per_cell, 1)) ** (1.0 / 3.0))))
        denom = max(float(self.scene_extent or 1.0), 1e-6)
        extent_norm = spatial_extent / denom
        self.grid_size = max(4, min(64, int(initial_grid_size * max(extent_norm, 1e-6))))
        ...
        w = self.merge_weights if self.merge_weights is not None else torch.ones_like(lin, dtype=torch.float32, device=self.get_xyz.device)
        counts.scatter_add_(0, lin, w)
        nz = counts.float(); m, s = nz.mean(), nz.std()
        self.std_threshold_factor = max(0.2, (2.0 * (s / max(m, 1e-6))).item())

    def _region_labels(self):
        if self.get_xyz.shape[0] == 0: return None, None, None
        min_xyz = self.get_xyz.min(dim=0).values
        max_xyz = self.get_xyz.max(dim=0).values
        G = int(self.grid_size)
        step = torch.clamp((max_xyz - min_xyz) / max(G, 1), min=1e-6)
        ...
        nz = counts[counts > 0]
        if nz.numel() == 0:
            mid = torch.ones(self.get_xyz.shape[0], dtype=torch.int8, device=self.get_xyz.device)
    ...
    @torch.no_grad()
    def _fast_merge_and_apply(self, region_labels, lin, counts,
                              merge_threshold_xyz, merge_threshold_color, merge_threshold_scale,
        ...
        unique_lin, counts_per_cell = torch.unique_consecutive(lin_sorted, return_counts=True)
        seg_ids = torch.repeat_interleave(torch.arange(unique_lin.numel(), device=device), counts_per_cell)
        idx_within = torch.arange(N, device=device) - torch.repeat_interleave(
            torch.cumsum(torch.cat([torch.tensor([0], device=device), counts_per_cell[:-1]]), dim=0),
            counts_per_cell
        )
        posA_sorted = torch.nonzero((idx_within % 2 == 0) & (idx_within + 1 < counts_per_cell[seg_ids]), as_tuple=False).squeeze(-1)
        if posA_sorted.numel() == 0: return
        A = order[posA_sorted]; B = order[posA_sorted + 1]
        rA = region_labels[A]
        scale = torch.ones_like(rA, dtype=torch.float32)
        scale = torch.where(rA == 0, torch.full_like(scale, self.merge_scale_sparse), scale)
        scale = torch.where(rA == 2, torch.full_like(scale, self.merge_scale_dense), scale)
        ...
        m = (dist < (merge_threshold_xyz * scale)) & (col < (merge_threshold_color * scale)) & (sca < (merge_threshold_scale * scale))
        if not torch.any(m): return
        ...
        a = A[idx_accepted]; b = B[idx_accepted]
        if a.numel() == 0: return
        ...
        merged_mask = torch.zeros(N, dtype=torch.bool, device=device); merged_mask[a] = True; merged_mask[b] = True
        ...
        if "importance" in optim:
            self._importance = optim["importance"]
        if self.max_radii2D.shape[0] < self._xyz.shape[0]:
            self.max_radii2D = torch.cat([self.max_radii2D, torch.zeros(new_xyz.shape[0], device=device)], dim=0)
        new_idx = torch.arange(self._xyz.shape[0] - new_xyz.shape[0], self._xyz.shape[0], device=device)
    ...
    def refine_and_prune(self, merge_threshold_xyz, merge_threshold_color, merge_threshold_scale, prune_percent, radii):
        if self.get_xyz.shape[0] == 0: return
        N = self.get_xyz.shape[0]
        if radii.shape[0] > N: radii = radii[:N]
        if self.max_radii2D.shape[0] != N: self.max_radii2D = torch.zeros(N, device=self.get_xyz.device)
        self.set_dynamic_thresholds()
        region_labels, lin, counts = self._region_labels()
        if region_labels is None: return
        with torch.no_grad():
            opacity_mask = self.get_opacity.squeeze(-1) < self.opacity_threshold
        if opacity_mask.any():
            opacity_mask = opacity_mask[:self.get_xyz.shape[0]]
            old = self.max_radii2D.clone(); self.prune_points(opacity_mask)
        ...
        prune_cfg = {0: self.prune_percent_sparse, 1: prune_percent if prune_percent is not None else self.prune_percent_mid, 2: self.prune_percent_dense}
        global_prune_mask = torch.zeros(self.get_xyz.shape[0], dtype=torch.bool, device=self.get_xyz.device)
        for cls in (0,1,2):
            idx = torch.nonzero(region_labels == cls, as_tuple=False).squeeze(-1)
            if idx.numel() < self.min_points_per_region: continue
        ...
        if global_prune_mask.any():
            ...
            prune_candidates = torch.nonzero(global_prune_mask, as_tuple=False).squeeze(-1)
            ...
            if cap > 0 and prune_candidates.numel() > cap:
                ...
                m = torch.zeros_like(global_prune_mask); m[prune_candidates[keep_idx]] = True
                global_prune_mask = m
            old = self.max_radii2D.clone(); self.prune_points(global_prune_mask)
            if self.max_radii2D.shape[0] != self.get_xyz.shape[0]:
                self.max_radii2D = old[~global_prune_mask][:self.get_xyz.shape[0]]
            region_labels, lin, counts = self._region_labels()
            if region_labels is None: return
        self._fast_merge_and_apply(region_labels, lin, counts,
                                   merge_threshold_xyz, merge_threshold_color, merge_threshold_scale,
                                   merge_cap_ratio=0.02)

    # ---- floater cleanup & adaptive cap ----
    def floater_cleanup(self, visibility_threshold, grad_threshold, opacity_threshold, importance_threshold,
                        max_fraction, distance_factor=1.5):
        num_points = self.get_xyz.shape[0]
        ...
        with torch.no_grad():
            ...
            if opacity_threshold > 0:     base_mask &= (opacity <= opacity_threshold)
            if importance_threshold > 0:  base_mask &= (importance <= importance_threshold)
            if grad_threshold > 0:        base_mask &= (grad_norm <= grad_threshold)
            center = self._xyz.mean(dim=0, keepdim=True)
            distances = torch.norm(self._xyz - center, dim=1)
            far_mask = distances >= (max(float(self.scene_extent or 1.0),1e-6) * distance_factor)
            detail_scale_threshold = max(0.02 * max(float(self.scene_extent or 1.0),1e-6), 1e-5)
            ...
            prune_cap = max(1, int(num_points * (max_fraction if max_fraction>0 else 0.05)))
            if idx.numel() > prune_cap:
                _, order = torch.topk(importance[idx], k=prune_cap, largest=False)
                idx = idx[order]
            mask = torch.zeros(num_points, dtype=torch.bool, device=device); mask[idx] = True
    ...
    def final_prune(self, threshold=0.05):
        with torch.no_grad():
            ...
            if mask.sum().item() > 0: self.prune_points(mask, source='final')
...
def compute_multiscale_l1(pred, tgt, scales):
    ...
    loss = torch.zeros((), device=pred.device, dtype=pred.dtype); wsum = 0.0
    for i, s in enumerate(uniq):
        w = 1.0 / (2 ** i)
        if s == 1.0: p, q = pred, tgt
        else:
        ...
        loss = loss + w * F.l1_loss(p, q); wsum += w
    return loss / max(wsum, 1e-6)

def gradient_consistency_loss(pred, tgt):
    def g(img): return img[..., :, 1:] - img[..., :, :-1], img[..., 1:, :] - img[..., :-1, :]
...
def _dyn_render_scale(iteration: int):
    if iteration < 4000: return 0.6
    if iteration < 10000: return 0.75
    return 1.0

def _copy_attr(dst, src, name_list):
    for n in name_list:
        if hasattr(src, n):
            try: setattr(dst, n, getattr(src, n))
            except Exception: pass

def make_scaled_camera(vp, scale: float):
    if scale >= 0.999: return vp
    class _CamProxy: pass
    new = _CamProxy()
    _copy_attr(new, vp, [
        "FoVx","FoVy","tan_fovx","tan_fovy",
        "world_view_transform","projection_matrix","full_proj_transform","world_view_proj",
        "camera_center","camera_matrix","viewmatrix","projmatrix","T",
        "fx","fy","cx","cy","intrinsics","distortion_params",
        "background","uid","time","shutter"
    ])
    ...
    if hasattr(vp, "viewport_size"):
        try: new.viewport_size = torch.tensor([newW, newH], dtype=torch.int32, device="cuda")
        except Exception: pass
    _copy_attr(new, vp, ["colmap_id","aid","sid","exif","path","name","frame_id","dataset_index",
                         "white_background","near","far","exposure","tone_mapping"])
    return new
...
def prepare_output_and_logger(args):
    ...
    flat_header = ['Iteration','SSIM','L1','PSNR','LPIPS','Loss','FPS','IterTime','Gaussians']
    flat_writer = csv.DictWriter(flat_file, fieldnames=flat_header); flat_writer.writeheader()
...
def measure_model_sizes(output_dir):
    ...
    for model_dir in glob.glob(model_dir_pattern):
        ...
        total_size = sum(os.path.getsize(p) for p in glob.glob(os.path.join(model_dir, "*.ply")))
...
def validation_loop(iteration, scene, gauss, pipe, bg, depth_est):
    ...
    for vcam in tqdm(cams, desc="[Val]"):
        ...
        start = time.time()
        with torch.no_grad(), suppress_debug_lines(("DEBUG:", "EARLY DEBUG:", "RASTERIZER DEBUG:")):
        ...
        if _HAS_LPIPS: lpips_t += _lpips(_ensure_batch_dim(img), _ensure_batch_dim(gt)).mean().double().item()
...
# ----------------------- Training -----------------------
DEFAULT_FINAL_ITERS = 30000
WARMUP_L2_ONLY_ITERS = 5000
POST_DSSIM = 0.2; POST_L1 = 0.3; POST_L2 = 0.7
...
def training(dataset, opt, pipe, test_iters, save_iters, ckpt_iters, checkpoint, debug_from):
    ...
    first_iter = 0
    tb_writer, csv_writer, csv_file, flat_writer, flat_file = prepare_output_and_logger(dataset)
    ...
    if hasattr(opt, "multiscale_scales"):
        ...
        try:
            if isinstance(raw, str): scales = [float(s.strip()) for s in raw.split(",") if s.strip()]
        except Exception: pass
    ...
    for it in range(first_iter + 1, opt.iterations + 1):
        ...
        with suppress_debug_lines(("DEBUG:", "EARLY DEBUG:", "RASTERIZER DEBUG:")):
            pkg = render(vcam_scaled, gauss, pipe, bg, importance_scores=torch.sigmoid(gauss.get_importance))
        img, depth_r, vis = pkg["render"], pkg["depth"], pkg["visibility_filter"]
        gt = vcam_scaled.original_image.cuda()

        # sRGB metrics
        pred = to_chw01(img); tgt = to_chw01(gt)
        if pred.shape[-2:] != tgt.shape[-2:]:
            tgt = F.interpolate(tgt, size=pred.shape[-2:], mode="bilinear", align_corners=False)
        l1v = l1_loss(pred, tgt); l2v = F.mse_loss(pred, tgt); ssim_v = ssim(pred, tgt)
        if it < WARMUP_L2_ONLY_ITERS:
            lam_l1, lam_l2, lam_dssim = 0.0, 1.0, 0.0
        else:
            lam_l1, lam_l2, lam_dssim = POST_L1, POST_L2, getattr(opt, "lambda_dssim", POST_DSSIM)
        multiscale_loss = compute_multiscale_l1(pred, tgt, scales) if getattr(opt, "lambda_multiscale", 0.0) > 0 else torch.zeros((), device=pred.device)
        grad_loss       = gradient_consistency_loss(pred, tgt)      if getattr(opt, "lambda_grad", 0.0)        > 0 else torch.zeros((), device=pred.device)
        moment_loss     = color_moment_loss(pred, tgt)              if getattr(opt, "lambda_color_moment", 0.0)> 0 else torch.zeros((), device=pred.device)
        lpips_metric = _lpips(pred, tgt).mean() if _HAS_LPIPS else torch.zeros((), device=pred.device)
        ...
        if gtd is not None:
            ...
            if unc is not None and unc.shape != rd.shape:
                unc = F.interpolate(unc.unsqueeze(0).unsqueeze(0), size=rd.shape, mode='bilinear', align_corners=False).squeeze()
            mask = (gtd>1e-6) & (rd>1e-6)
            if mask.sum()>0:
                gtv, rdv = gtd[mask], rd[mask]
                t1, t2 = torch.median(gtv), torch.median(rdv)
                ...
                if unc is None:
                    ...
                    w = torch.exp(-unc[mask] / tau)
                w = w / (w.mean() + 1e-6)
                depth_loss = (w * F.huber_loss(rd_al, gtv, reduction='none')).mean()

        total = lam_l1*l1v + lam_l2*l2v + lam_dssim*(1.0-ssim_v) \
              ...
              + getattr(opt,"lambda_importance",0.01)*torch.sigmoid(gauss.get_importance).mean() \
              + lpips_loss
        ...
        with torch.no_grad():
            ...
            if it < opt.densify_until_iter:
                ...
                if it > opt.densify_from_iter and it % fast_interval == 0:
                    gauss.densify_and_prune(opt.densify_grad_threshold, 0.003, scene.cameras_extent, None, radii)
                ...
                if it % 200 == 0:
                    gauss._ensure_point_buffers()
                    radii = align_tensor_to_length(pkg["radii"], gauss.get_xyz.shape[0])
                    gauss.refine_and_prune(merge_threshold_xyz=0.01, merge_threshold_color=0.12,
                                           merge_threshold_scale=0.12, prune_percent=0.005, radii=radii)
                if it % 500 == 0:
                    low_imp = (torch.sigmoid(gauss.get_importance) < 0.05).squeeze()
                    if low_imp.any(): gauss.prune_points(low_imp, source='final')

            if it >= getattr(opt,"floater_cleanup_start",15000) and it % getattr(opt,"floater_cleanup_interval",400) == 0:
                gauss.floater_cleanup(
                    visibility_threshold=getattr(opt,"floater_visibility_threshold",2),
                    grad_threshold=getattr(opt,"floater_grad_threshold",5e-4),
                    opacity_threshold=getattr(opt,"floater_opacity_threshold",0.04),
                    importance_threshold=getattr(opt,"floater_importance_threshold",0.35),
        ...
        if do_eval:
            ...
            if os.path.isdir(pc_dir):
                size_mb = sum(os.path.getsize(p) for p in glob.glob(os.path.join(pc_dir, "*.ply"))) / (1024*1024)
            if size_mb > 0:
                base_row['Model_Size(MB)'] = f"{size_mb:.2f}"

        # write eval/detailed row
        ...
        if it in save_iters: scene.save(it)
        if it in ckpt_iters: torch.save((gauss.capture(), it), scene.model_path + f"/chkpnt{it}.pth")

        bar.set_postfix({"Loss": f"{float(total.item()):.6f}",
                         "Points": f"{gauss.get_xyz.shape[0]}",
                         "FPS": f"{(1000.0/max(elapsed,1e-3)):.2f}"})
        bar.update(1)

    bar.close(); csv_file.close(); flat_file.close(); print("[Train] complete.")
...
if __name__ == "__main__":
    ...
    # test/save aligned; we'll override iterations to 20k
    safe_add_argument(parser, "--test_iterations", nargs="+", type=int, default=[7000, 15000, DEFAULT_FINAL_ITERS])
    safe_add_argument(parser, "--save_iterations", nargs="+", type=int, default=[DEFAULT_FINAL_ITERS])
    ...
    # ✅ Hard-fix iterations to 20k
    args.iterations = DEFAULT_FINAL_ITERS
    if DEFAULT_FINAL_ITERS not in args.save_iterations: args.save_iterations.append(DEFAULT_FINAL_ITERS)
    if DEFAULT_FINAL_ITERS not in args.test_iterations: args.test_iterations.append(DEFAULT_FINAL_ITERS)
    ...
    # 🔒 Force densify phase to stop at 10k iters
    setattr(opt_params, "densify_until_iter", 10000)
    print(f"[Sched] densify_until_iter set to {getattr(opt_params,'densify_until_iter',None)}")

    # map a few CLI → opt
    for k in ["lambda_depth","lambda_lpips","lambda_importance","uncertainty_tau","eval_interval",
              "max_clone_points_per_iter","max_split_points_per_iter","log_densify_clamps","max_gaussians",
              "floater_cleanup_start","floater_cleanup_interval","floater_visibility_threshold","floater_grad_threshold",
              "floater_opacity_threshold","floater_importance_threshold","floater_cleanup_max_ratio","floater_distance_factor",
              "lambda_multiscale","lambda_grad","lambda_color_moment","flat_log_interval"]:
        if hasattr(args,k): setattr(opt_params, k, getattr(args,k))
